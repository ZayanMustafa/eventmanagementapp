    ...
    <div className="bg-surface rounded-xl p-6 border border-gray-700">
      ...
      {isSuccess ? (
        <div className="text-center py-8">
          ...
          <p className="text-secondary">Our team will contact you within 48 hours</p>
        </div>
      ) : (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {EVENT_TYPES.map((type) => (
                  <button
                    key={type.name}
                    type="button"
                    onClick={() => setFormData({...formData, eventType: type.name})}
                  ...
                  >
                    ...
                    <span className="text-xs text-secondary mt-1">{type.desc}</span>
                  </button>
                ))}
              </div>
            </div>
